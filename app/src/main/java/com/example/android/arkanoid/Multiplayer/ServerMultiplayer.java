...
public class ServerMultiplayer implements Runnable, TimerListener {
    ...
    private void setup(){
        ...
        while(Math.abs(this.direzionePalla.getPosX()) == 1 || Math.abs(this.direzionePalla.getPosY()) == 1) {
            this.direzionePalla = new Vector2D(1, 0).ruotaVettore((float)(Math.random() * 360));
        }
